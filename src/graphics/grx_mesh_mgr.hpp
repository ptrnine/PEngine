...
public:
    grx_mesh_mgr(const core::config_manager& config_manager, const core::string& mgr_tag);
...
private:
    static shared_ptr<grx_mesh> load_mesh(string_view path, bool instanced, grx_texture_mgr<4>* texture_mgr);

private:
    std::mutex                             _meshes_mutex;
    hash_map<string, shared_ptr<grx_mesh>> _meshes;
    core::shared_ptr<grx_texture_mgr<4>>   _texture_mgr;
    string                                 _models_dir;
};
